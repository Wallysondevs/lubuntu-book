    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      ...
      >
        ...
        <h1 className="text-4xl sm:text-5xl font-bold text-py-blue dark:text-py-yellow mb-4">
          Lubuntu: Do Zero ao Domínio
        </h1>
        <p className="text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto">
          Um livro completo em português com <strong>{chapters.length} capítulos</strong> práticos —
          do primeiro <code className="text-py-blue dark:text-py-yellow">ls ~</code> até personalização, otimização para hardware leve e projetos reais.
        </p>
        <div className="mt-8 flex flex-wrap gap-3 justify-center">
          ...
          <Link href="/c/baixar-iso" className="inline-flex items-center gap-2 bg-py-yellow hover:bg-py-yellow-dark text-py-blue-dark px-6 py-3 rounded-lg font-semibold shadow transition-colors">
            Instalar Lubuntu <ArrowRight size={18} />
          </Link>
        </div>
      </motion.section>
      ...
      <section className="mb-14 bg-slate-900 text-slate-100 rounded-2xl p-6 sm:p-8 shadow-xl">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-mono text-py-yellow">terminal.sh</span>
            <span className="text-xs text-slate-400">Lubuntu · LXQt</span>
          </div>
          <pre className="text-sm leading-relaxed font-mono overflow-x-auto"><code>{`# Atualizar o sistema e instalar um app
  sudo apt update && sudo apt upgrade -y
  sudo apt install vlc gimp libreoffice

  # Ver quanto de RAM o Lubuntu está usando
  free -h

  # Listar processos consumindo mais memória
  ps aux --sort=-%mem | head -5

  # Ver versão do Lubuntu instalada
  lsb_release -a

  # Pronto! Você acabou de mexer no terminal como um pro.`}</code></pre>
      </section>
